...
1. The utterance-wise separaiton data is downloaded alone with the continuous evaluation data with 

    ```
    ./dataprep/scripts/dataprep_separation.sh
    ```

    Under each exp/data/separation_result/utterance_separation/overlap_ratio_*, you will find utterance\_\*\__{0,1}.wav. For example, utterance\_0\_{0,1}.wav are the two-channel signals generated by the utterance-wise separation algorithm applied to the original 7-channel utterance\_0.wav. 

2. Then, the asr and wer  calculation can be performed with following step
    ...
    After this step the word error rate of the utterance evaluation will be displayed as
