#!/usr/bin/env python
# -*- coding: utf-8 -*-
import argparse, os, datetime, copy, re, string
...
def read_scp(scpfile, encoding='utf-8'):
    ...
    with open(scpfile, encoding=encoding) as infile:
        ...
        for lineno, line in enumerate(infile):
            ...
            trs.append((start_time, end_time, tr))

    trs = sorted(trs, key=lambda x: x[0])
    spkr = 0
    bucketed_trs = defaultdict(list)

    # Create streams of overlap-free utterances. 
    while len(trs) > 0:
        spkr_str = string.ascii_uppercase[spkr]
        for sent in trs:
            if len(bucketed_trs[spkr_str]) == 0:
                bucketed_trs[spkr_str].append(sent)
            else:
                cur_start_time = sent[0]
                prev_end_time = bucketed_trs[spkr_str][-1][1]
                if cur_start_time >= prev_end_time:
                    bucketed_trs[spkr_str].append(sent)

        # Remove the sentences that were thrown into the bucket. 
        trs = list(set(trs) - set(bucketed_trs[spkr_str]))
        trs = sorted(trs, key=lambda x: x[0])
        spkr += 1
