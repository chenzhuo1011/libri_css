...
def main(args):
    ptrn = re.compile('(\S+_\d+)_(\d+)\D\S*\D(\d+)_(\d+)')
    # overlap_ratio_0.0_sil0.1_0.5_session0_actual0.0_seg_7_0_232639_338560
    #ptrn = re.compile('(\S+_\d+)_(\d+)_(\d+)_(\d+)')
    ptrn = re.compile('(\S+_\d+)_(\d+)_(\d+)')
    ...
    with open(args.inputfile) as f:
        for line in f:
            ...
            #m = ptrn.match(fields[0].split('_', maxsplit=7)[-1])
            m = ptrn.match(fields[0].split('_', maxsplit=7)[-1])
            if m is None:
