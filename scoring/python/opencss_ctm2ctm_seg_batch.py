...
def main(args):
    ...
    for subdir in subdirs:
        ...
        for srcfile in ctmfiles:
            ...
            if args.noseparation:
                execute_python_script('opencss_ctm2ctm_baseline', ' '.join(opts).split())
            else:
                execute_python_script('opencss_ctm2ctm_seg', ' '.join(opts).split())
...
def make_argparse():
    ...
    parser.add_argument('--model_name', metavar='<str>', required=True, 
                        help='AM name.')
    parser.add_argument('--method_name', metavar='<str>', required=True,
                        help='Method name.')
    parser.add_argument('--config_name', metavar='<str>', required=True,
                        help='Configuration name.')
    parser.add_argument('--noseparation', action='store_true', 
                        help='Use this option for no-separation results.')
